...
static struct ufile_error
download1(const char *bucket_name,
         const char *key,
         size_t start_position,
         FILE *f,
         char *buf,
...
struct ufile_error ufile_download1(const char *bucket_name, const char *key, FILE *file, size_t *return_size){
    return download1(bucket_name, key, 0, file, NULL, 0, return_size);
}
